...
class DecoderBlock():
    def __init__(self, optimizer, d_model=512, d_ff=2048, num_attention_heads=8, dropout_rate=0.1, mask1=None, mask2=None, data_type=np.float32, training=True):
    ...
    def forward(self, target, source):
        ...
        attention_output1 = self.multi_head_att1.forward(q, k, v, self.training)
        attention_output1 = self.dropout.forward(attention_output1, self.training)
        target_ = self.layernorm1.forward(target + attention_output1)
        q, k, v = source, source, target_
        attention_output2 = self.multi_head_att2.forward(q, k, v, self.training)
        attention_output2 = self.dropout.forward(attention_output2, self.training)
        x = self.layernorm2.forward(target_ + attention_output2)
        x_ = self.ffn.forward(x, self.training)
        x_ = self.dropout.forward(x_, self.training)
